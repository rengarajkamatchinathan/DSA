package graph.other;

import java.util.ArrayList;
import java.util.List;

public class _2_articulation_point {
    static int timer = 0;
    static void dfs(int node, int par, List<List<Integer>> adj, int time_in[], int low[], boolean vis[], List<Integer> res){
        time_in[node] = low[node] = timer++;
        vis[node] = true;

        //edge
        int child_cnt = 0;
        for(int child:adj.get(node)){
            if(child==par)continue;

            if (!vis[child]) {
                dfs(child, node, adj, time_in, low, vis, res);
                low[node] = Math.min(low[node], low[child]);
                if (low[child]>=time_in[node] && par!=-1) {
                    res.add(node);
                }
                child_cnt++;
            }
            else{
                low[node] = Math.min(low[node], time_in[child]);
            }
        }
        
        if (child_cnt>1 && par==-1) {
            res.add(node);
        }
    }
    public static void main(String[] args) {
        int N = 12, connection[][]={{0,1},{0,2},{0,3},{2,3},{2,4},{2,5},{4,6},{5,6}};
        //Result: 0,2

        List<List<Integer>> adj = new ArrayList<>();
        for (int i = 0; i < N+1; i++) adj.add(new ArrayList<>());
        for(int a[]:connection){
            adj.get(a[0]).add(a[1]);
            adj.get(a[1]).add(a[0]);
        }
        List<Integer> res = new ArrayList<>();
        int timer_in[] = new int[N+1];
        int low[] = new int[N+1];
        boolean vis[] = new boolean[N+1];
        dfs(1, -1, adj, timer_in, low, vis, res);
        System.out.println(res);
    }
}
