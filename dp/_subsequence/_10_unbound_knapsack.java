...
public class _10_unbound_knapsack {
    static int memo(int ind, int wt, int weights[], int vals[], int dp[][]){
        ...
        int pick = 0;
        if(weights[ind]<=wt){
            pick = vals[ind] + memo(ind-1, wt-weights[ind], weights, vals, dp);
        }
    ...
    static int tab(int W, int weights[], int vals[]){
        ...
        for(int ind=1;ind<dp.length;ind++){
            for(int wt=0;wt<=W;wt++){
                ...
                int pick = 0;
                if(weights[ind]<=wt){
                    pick = vals[ind] + dp[ind-1][wt-weights[ind]];
                }
